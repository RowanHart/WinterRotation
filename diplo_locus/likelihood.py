...
class SelHmm:
    ...
    @staticmethod
    def getInitDistribution (yGrid, gridWeights, Ne, s1, s2, mAlpha, mBeta, initCond, initProbs=None,
        ...
        if (initCond in ["statBeta", "statBetaSel"]):
            ...
            assert (initMBeta > 0), "Stationary initial condition needs positive mutation rate. Or supply one just for initial with 'initMBeta'"
        ...
        if (initCond == "initFreq"):
            ...
            initIsFloat = isinstance (initFreq, (float, numpy.floating))
            if (initIsFloat or (len(initFreq) == 1)):
                # one 
                if (not initIsFloat):
                    initFreq = initFreq[0]
                assert (initFreq >= 0)
        ...
        # easy enough to have it here
            ...
            initial = gridWeights
        elif (initCond == "distribution"):
            assert (initProbs is not None)
            assert (type(initProbs) in [list, numpy.ndarray])
            initial = numpy.array (initProbs)
            assert (len(initial.shape) == 1)
            assert (len(initial) == len(yGrid))
            assert ((initial.min() >= 0) or (numpy.isclose (initial.min(), 0)))
            assert (numpy.isclose (numpy.sum (initial), 1))
            initial = numpy.clip (initial, 0, 1)
        else:
    ...
    def __init__ (self, Ne=None, s1=None, s2=None, mAlpha=None, mBeta=None, initCond=None,
                    ...
                    numStates=501, deltaT=1, emissionType="integer", transitionType="constant", selectionChangeTimes=None,
                    selectionGridObject=None, initProbs=None):
        ...
        self.initDist = SelHmm.getInitDistribution (yGrid, gridWeights, Ne, s1, s2, mAlpha, mBeta, initCond, initProbs=initProbs,
...
def surfaceOneD (times, samplesSizes, samples, selDirection, selCoefficients, **kwargs):
    ...
    selDirection = numpy.array (selDirection)
    ...
    assert (samples.shape == samplesSizes.shape), f"{samples.shape} == {samplesSizes.shape}"
